...
import { assets, payment, purchase } from "@/lib/db/schema";
...
export async function createPaypalOrderAction(assetId: string) {
  ...
  const [getAsset] = await db.select().from(assets).where(eq(assets.id, assetId));
  if (!getAsset) {
...
export async function getAllUserPurchasedAssetsAction() {
  ...
  try {
    const userPurchases = await db
      ...
      .from(purchase)
      .innerJoin(assets, eq(purchase.assetId, assets.id))
